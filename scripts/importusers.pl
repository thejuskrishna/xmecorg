...
use lib "/home/cherian/xmec.org/scripts";
use xmec_config;
use strict;
use DBI;
...
my $from_emailid = "webmasters\@xmec.org";
...
my @mand = ('ROLL_NO', 'BRANCH','YR_OF_PASSING', 'FIRST_NAME', 'PERS_EMAIL');
...
sub delete_address
{
    ...
    if (!$dryrun) {
        $dbh->do(qq{
            DELETE FROM xmec_address WHERE user_id = '$uid' AND
...
sub delete_user
{
    ...
    if (!$dryrun) {
        $dbh->do(qq{
            DELETE FROM xmec_user WHERE id = '$uid'}); # or die $dbh->errstr;
    }
}
...
sub delete_auth
{
    ...
    if (!$dryrun) {
        $dbh->do(qq{
            DELETE FROM xmec_auth WHERE user_id = '$uid'}); # or die $dbh->errstr;
    }
}
