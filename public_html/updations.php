...
<table border=0>
	...
	for ($i = 0; $i < sizeof($wrk); $i++) {
		...
		$batch = $res[4];
		echo '<tr>
						<td>&nbsp;<A HREF="View '.$res[1].'\'s profile ('.$batch.' batch)" onClick="javascript:open(\'/profile_admin.php?.s=1&id='.rawurlencode($res[0]).'\',\'\',\'scroll=yes,scrollbars=yes\');return false;" class=link>' . $res[1] . 
'</A></td>
						<TD CLASS=LINKH>'.$res[3].' '.$batch.'&nbsp;</TD>
