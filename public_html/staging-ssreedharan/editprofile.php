...
//	include 'xmec.inc';
 ...
 $img="profile/thumbnails/".str_replace("/","_",$user->get('id')).".jpg";
	if ($action == "update") {
		...
		$user->set('chapter_id', $_POST['CBChapter']);
		...
		$addr->set('street', $_POST['TBPStr']);
		$addr->set('area', $_POST['TBPArea']);
		$addr->set('city', $_POST['TBPCity']);
		$addr->set('state', $_POST['TBPState']);
		$addr->set('country', $_POST['TBPCountry']);
		$addr->set('postal_code', $_POST['TBPPin']);
		$addr->set('telephone_no', $_POST['TBHPhone']);
		...
		$addr->set('house_name', $_POST['TBCname']);
        ...
        if(isset($_POST["BHSubmit"]))
        {
            for($cnt=0;$cnt <= $_POST["num"]; $cnt+=1)
            {       $d=array();
		if($_POST["univ".$cnt]=="")
		{
		$user->removedu($_POST["row_id".$cnt]);
		}
		else
		{
                $d["user_id"]=$user->id;
...
function check()
{
 ...
 // } else {
   ...
   document.Fcr.action = "<?=$PHP_SELF."?target=editprofile.php"?>";
...
<TABLE align=left border=0 cellPadding=10 cellSpacing=0 width="100%">
 <TR>
	<TD align=center height=35 class=head>PROFILE IMAGE</TD>
 </TR>
 <TR>
	<TD align=center>
...
Upload New Image
<?
 if( $_POST['submit']=="Upload" && $_FILES['uploaded_image']['tmp_name']!="" ) {
...
<TD width=150><INPUT class=box style="WIDTH: 144px" name=TBDob value="<?=htmlentities($user->get('date_of_birth'))?>"></TD>
<TD class=name width=100>Sex</TD>
<TD width=150><SELECT  class=cbox style="WIDTH: 150px" name=CBSex >
...
<TD><SELECT  class=cbox style="WIDTH: 150px" name=CBMstatus >
	<OPTION value="S">Single</OPTION>
	<OPTION <?if ($user->get('marital_status') == "M") echo "selected";?> value="M">Married</OPTION>
	</SELECT></TD>
<TD height=35 class=fbody>Spouse Name</TD>
<TD><INPUT class=box style="WIDTH: 144px" name=TBSpouse value="<?=htmlentities($user->get('aol'))?>"></TD>
</TR>
</TABLE>
...
<TD height=35 class=fbody>Field of Work</TD>
    <TD width=150><INPUT class=box style="HEIGHT: 19px; WIDTH: 144px" name=CBWork  value="<?=htmlentities($user->get('work_type'))?>"></TD>
</TR>
<TR>
...
<TD width=150><INPUT class=box style="HEIGHT: 19px; WIDTH: 144px" name=TBCname value="<?=htmlentities($user->get('company'))?>"></TD>
...
<INPUT  name=BSubmit type=button value=Update onClick="javascript:check();" style="HEIGHT: 24px; WIDTH: 96px">
</TR>
<tr><td>&nbsp;</td></tr>
<tr><td>&nbsp;</td></tr>
</TABLE>
...
?>
            ...
            <td>
<?
            echo "<SELECT class=cbox style=\"width:144px\" name=end_year".$i." value=\"".$college->end_year."\">";
?>
            <OPTION value="">--- Select ---</OPTION>
<?php
            for ($y=1989; $y<=2020; $y++)
...
?>
      <INPUT  name=BHSubmit type=submit value=Update  style="HEIGHT: 24px; WIDTH: 96px" />
      </div>
</form>
</div>
<BR>
