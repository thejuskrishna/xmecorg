...
class XMEC_poll {
  ...
  function XMEC_poll($question = null) {
    ...
    if (DB::isError($queryHandle)) {
      $pollError = "1.Database Query Failed.<BR>";
      return;
    }
	...
	function hasUserVoted($user = null) {
		...
		if (DB::isError($queryHndl)) {
			$pollError = "2.Database Query failed.<BR>";
			return (-1);
		}
...
function getOpenPolls() {
  ...
  $queryHandle = $dbh->query($query);
  if (DB::isError($queryHandle)) {
    $pollError = "3.Database Query Failed.<BR>";
		echo 'DB error<br>';
    return $result;
  }
...
function getAllPolls() {
  ...
  $dbh =& XMEC::getDB();
  $query =
    "SELECT * , DATE_FORMAT(start_date,'%b %D,%Y'), DATE_FORMAT(end_date, '%b %D,%Y') FROM poll_questions";
...
function getPoll($pollID) {
  ...
  $queryHandle = $dbh->query($query);
  if (DB::isError($queryHandle)) {
    $pollError = "4.Database Query Failed.<BR>";
    return $poll;
  }

  $question = $queryHandle->fetchRow();
  if (!is_array($question)) {
   $pollError = "5.Database Query Failed.<BR>";
   return $poll;
  }
...
function createPoll($pollTitle,$pollQuestion,
  ...
  $queryHandle = $dbh->query($query);
  if (DB::isError($queryHandle)) {
    echo "6.Database Query Failed.<BR>";
    return 1;
  }
    ...
    foreach ($answers as $ans) {
      $query = "INSERT INTO poll_answers VALUES( LAST_INSERT_ID()," . 
  		...
  		$queryHandle = $dbh->query($query);
  		if (DB::isError($queryHandle)) {
        echo "7. $query \n ".$queryHandle->getMessage()." Database Query Failed.<BR>";
				return 1;
      }
    }
		return 0;
...
function registerVote($pollID, $user, $ansSeq) {
  ...
  $queryHandle = $dbh->query($query);
  if (DB::isError($queryHandle)) {
    $pollError = "8.Database Query Failed.<BR>";
    return FALSE;
  }
  return TRUE;
}
