<?php
$secure_page=0;
$this_page="letters";
$no_search_menu = 1;
include 'header.php';
?>
<!--center starts-->
<TABLE cellSpacing=0 cellPadding=1 width=98% border=0>
  <TR>
  	<TD width=477 height=40 class=head><B>News >></B> Letters</TD>
  </TR>
  <TR>
  	<TD width=90% valign=top>
            <P class=body><IMG alt="pen"  src="images/pen.gif">&nbsp;&nbsp;<strong>Letters from XMECians</strong>
		<HR color="#dddddd" height="1" width="100%">
	<TABLE width=433>
		<TR><TD bgcolor=#eeeeee>&nbsp;&nbsp;&nbsp<A href="nba.php" class=link>Report on XMEC interaction with NBA</A></TD></TR>
		<TR><TD bgcolor=#E7F3FB>&nbsp;&nbsp;&nbsp;<A href="rem.php" class=link>XMECians fondly remember Dr. Sukulal</A></TD></TR>
				<TR><TD bgcolor=#eeeeee>&nbsp;&nbsp;&nbsp<A href="resume.php" class=link>Tips on Resume Drafting</A></TD></TR>
		<TR><TD bgcolor=#E7F3FB>&nbsp;&nbsp;&nbsp;<A href="bme.php" class=link>Careers Prospects after Biomedical Engineering</A></TD></TR>
		<TR><TD bgcolor=#eeeeee>&nbsp;&nbsp;&nbsp<A href="ms_letter.php" class=link>XMECians write on post-graduate studies in the USA</A></TD></TR>
		<TR><TD bgcolor=#E7F3FB>&nbsp;&nbsp;&nbsp;<A href="credits.php" class=link>People behind www.xmec.net</A></TD></TR>
		</TABLE>
            <P></P>

	</TD>
</TR>

</TABLE>

<BR><BR>

	<TD width=433>

		<P class=body><IMG src="images/pen.gif">&nbsp;&nbsp;<strong>Techno Trivia</strong>
		<HR color="#dddddd" height="1" width="100%">
	<TABLE width=433>
		<TR><TD bgcolor=#dddddd>&nbsp;&nbsp;&nbsp;<A href="chemistry.php" class=link>The Chemistry of Communication</A></TD>
		</TR></TABLE>
            <P></P>

	</TD>

  <TR><TD colspan=3><P><BR><CENTER><A HREF="mailto:webmasters@xmec.net?subject=[xmec.net] Letters to XMEC" class=flink >Add a Letter to XMEC site</A></CENTER><BR>
  </TD></TR>

<!--center ends-->
<?php
include 'footer.php';
?>
