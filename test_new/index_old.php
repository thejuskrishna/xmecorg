...
<TABLE cellSpacing=0 cellPadding=0 bgcolor="#8EBBE6" align=left border=0>
  <TR>
    <TD><A href="show('1')" ><img src="images/1_off.gif" name=img1 border=0></A></TD>
    <TD><A onClick="show('3')"><img src="images/3_off.gif" name=img3 border=0></A></TD>
...
<?php
		...
		<?php
			...
			$sqle = "select company from job_posts  order by post_id desc limit 0,1;";
					...
					if (DB::isError($r)) {
						...
						while (is_array($x = $r->fetchRow())) {
						  for ($i=0; $i<count($x); $i++) echo "<P><a href=post_job.php class=flink>$x[$i]</A><BR></P>";
	...
	<? else: ?>
  	...
  	<TD valign=top><BR>
  	<P class=title>Top XMEC Employers is the abbreviation for exMECians, the Alumni Association of Model Engineering College, Thrikkakara, Kochi. XMEC is an autonomous body representing thousand three hundred and more engineers who have earned their degrees in
