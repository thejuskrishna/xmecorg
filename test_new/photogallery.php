...
// echo "DEBUG: number: $number, start: $start, stop: $stop\n total: " . ($stop - $start) . "\n";
	...
	{	
		...
		foreach($languages as $thisoth)
		{	
			if ($lang == $thisoth["short"])
				echo "[" . $thisoth["full"] . "] ";
			else
				echo "<A HREF=\"$thisfile?lang=" . $thisoth["short"] . 
					"&action=gallery&pagenum=$pagenum\">[" . $thisoth["full"] . "]</A> ";
		}
		echo "</CENTER>";
  ...
  echo "<BR><CENTER><H2>" . $clickmestr[$lang] . "</H2></CENTER>";
  
  # Loop through all the images in the directory
	foreach($pictures as $thisoth)
	{
    $entry = $thisoth["name"];
		$total++;
