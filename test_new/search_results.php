<?php
   ...
   else
   {
	 $res =& XMEC::search($search_fil, $search_start, $search_count);
         $link="<A href=$PHP_SELF?n=".urlencode($search_fil['name'])."&w=".urlencode($search_fil['work_type'])."&c=".urlencode($search_fil['company'])."&y=".urlencode($search_fil['year'])."&b=".urlencode($search_fil['branch'])."&l=".urlencode($search_fil['location'])."&f=";
   }
